/*
	tracking function using opticalflowPyrLK
	2016.2.5
*/
...
bool LKTracker::trackf2f(const Mat& img1, const Mat& img2, std::vector<Point2f> &points1, std::vector<cv::Point2f> &points2){
	//implement c function cvCalcOpticalFlowPyLK() or Faster tracking function
...
void LKTracker::normCrossCorrelation(const Mat& img1, const Mat& img2, std::vector<Point2f>& points1, std::vector<Point2f>& points2){
	Mat rec0(10,10,CV_8U);
	Mat rec1(109,10,CV_8U);
	Mat res(1,1,CV_32F);
...
bool LKTracker::filterPts(std::vector<Point2f>& points1, std::vector<Point2f>& points2){
	//Get Error Medians
